 ...
 *
 * 推导做过一次逐项对账，确认每一支恰好少掉它该有的那些行盒、没有漏扣或扣重
 * （左：改动前的估值，右：现在的估值）：
 *
 * | 块 | 改前 | 改后 | 实减 | 应减 |
 * | --- | --- | --- | --- | --- |
 * | h1 / h2 / h3 | 88 / 78 / 65 | 61 / 51 / 38 | 27 | 27（各一个块尾行盒） |
 * | 单行段落 | 67 | 40 | 27 | 27 |
 * | 4 行缩进代码块 | 148 | 121 | 27 | 27 |
 * | 3 项列表 | 229 | 94 | 135 | 5 × 27 |
 * | 两行引用块 | 148 | 67 | 81 | 3 × 27 |
 * | 10 行 fenced 代码块 | 292 | 292 | 0 | 该支本就不含空行盒 |
 * | 4 行表格 | 280 | 177 | 103 | 4 × 19 + 27 |
 *
 * 表格那 103 单独核过：每行 19 是表格 HTML 里那些换行撑出的（见 `TABLE_ROW_PX`），
 * 再加块尾一个 27。
 *
 * 但对账只证明**推导自洽**，不证明数字对。这套值**尚未在真实浏览器里重新标定**，
 * 也没有任何测试在守它们——估算偏了不会报错，只会让滚动落点漂移。
 * 要复测的话，量 `.cm-md-block` 的边框盒即可。
 */
