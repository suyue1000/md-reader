 ...
 *
 * 差额里有 **27.2px 与「块 vs 连续」无关**：那是 `.cm-content` 的
 * `white-space: break-spaces` 继承进块 widget 之后，每个块尾部多出来的一个
 * 空行盒——`block-height.ts` 的模块注释里有完整记载，并写明给 `.cm-md-block`
 * 补一句 `white-space: normal` 就能消掉。
 *
 * 也就是说：**走偏的是屏幕，不是产物**。产物是正常的排版节奏（标题上宽下窄），
 * 屏幕反而下比上还宽。这是可修的，不是两种呈现方式的固有差别——只是修它要
 * 重新标定 `block-height.ts` 那套按当前排版量出来的常量，不该由导出改动顺手带出来。
 *
 * ## 本模块的职责
 *
