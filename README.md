...
接管的页面里也能**直接编辑并写回原文件**：按 `⌘E`（Windows / Linux 是 `Ctrl+E`）进入编辑态，会弹一次文件选择器，请你选中**正在看的这一篇**。这一步绕不过去——浏览器不允许网页凭地址取得某个文件的写入权限，句柄只能来自一次真实的选择。选择器会尽量开在离目标近的地方：按文件路径落到「桌面 / 文稿 / 下载」等系统目录那一支，之后再用会直接停在你上次选过的目录。但它**没法替你选中那个文件**——浏览器只认几个知名目录常量，不接受任意路径作为起始位置（否则网页就能探测你的文件系统结构）。

选中之后，停笔 800ms 自动写回，`⌘S` 立即写回。

注意这里的「允许访问文件网址」只管**读**；写回是另一回事，由上面那次选择单独授权。
...
**在接管的页面里按 `⌘S`，怎么变成下载了？**
说明还没授权写回。接管页面里的阅读器是跨源 iframe，浏览器禁止这种 iframe 弹出任何文件选择器，所以它既拿不到当前文件的句柄、也弹不出「另存为」，只能退到下载——文件落进下载目录，**原文件一个字节都没改**。按 `⌘E` 进入编辑态完成一次授权，之后就能直接写回了。
